import { ArrowLeft, CircuitBoard, Dot, ExternalLink, Fish, Laptop, Music, RotateCcw, Scan, Smartphone, TextCursor } from "lucide-react";
...
export default function ProjectDetail({ projectId, onBack }: ProjectDetailProps) {
  ...
  const projectsData: Record<string, ProjectData> = {
    ...
    "3": {
      ...
      category: "IoT et VR",
      description: "Maquette de poubelles connectées, avec un suivi du remplissage, de l'ouverture/fermeture, de la chute de celles-ci. Avec une application VR pour le suivi du camion-poubelle.",
      longDescription:
            ...
            <div className={`bg-gradient-to-br ${project.color} backdrop-blur-xl rounded-3xl border border-white/10 p-8 md:p-12`}>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                <div>
                  <div className="flex items-center space-x-4 mb-6">
                    <div className="w-20 h-20 bg-white/10 backdrop-blur-sm rounded-3xl flex items-center justify-center">
                      {project.icon && <project.icon className={`w-10 h-10 ${project.accentColor}`} />}
                    </div>
                    <div>
                ...
                <div className="relative">
                  <img src={project.image} alt={project.title} className="w-full h-80 object-cover rounded-3xl shadow-2xl" />
              ...
              {activeTab === "overview" && (
                <div className="space-y-8">
                  <div className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 p-8">
                    ...
                    <p className="text-white/80 leading-relaxed text-lg">{project.longDescription}</p>
                  </div>
                  ...
                  <div className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 p-8">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {project.technologies.map((tech, index) => (
                        <span key={index} className="px-4 py-2 bg-white/10 backdrop-blur-sm rounded-full text-white/80 border border-white/20 text-sm font-medium">
                          {tech}
                        </span>
                      ))}
                    </div>
                    ...
                    {iframeError ? (
                      <div className="absolute inset-0 flex items-center justify-center bg-white/5 backdrop-blur-sm">
                        <div className="text-center">
                          ...
                          <button onClick={refreshIframe} className="bg-white/20 backdrop-blur-sm border border-white/30 px-4 py-2 rounded-xl text-white hover:bg-white/30 transition-all duration-300">
                            Réessayer
                          </button>
                        </div>
                      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {project.screenshots.map((screenshot, index) => (
                      <div key={index} className="relative group">
                        <img src={screenshot} alt={`Capture ${index + 1}`} className="w-full h-64 object-cover rounded-2xl shadow-lg group-hover:scale-105 transition-transform duration-300" />
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-white/60">Développeur/Equipe de développeurs</span>
                  ...
                  {project.compatibility.map((device, index) => (
                    <div key={index} className="flex items-center space-x-3 p-3 bg-white/5 rounded-2xl border border-white/10">
                      ...
                      <span className="text-white font-medium">
                        {device === "phone" ? "Téléphone" : device === "computer" ? "Ordinateur" : device === "linux" ? "Linux" : device === "ar" ? "Réalité Augmentée" : "Autre"}
                      </span>
